...
mdp <-
  function(data,
    ...
    }  else if (!("Sample" %in% names(pdata) &&
                  "Class" %in% names(pdata))) {
    ...
    pdata <-
      pdata[as.character(pdata$Sample) %in% colnames(data), ]  # Only keep the samples that have both pdata and data
    rownames(pdata) <- pdata$Sample
    data <-
      data[, as.character(pdata$Sample)]  # Expression data has c(1) and samples
    ...
    Zscore[, -idx[[control_idx]]] <-
      apply(data[, -idx[[control_idx]]] , 2, function(x)
        Z(x, dataHealth, std))
    
    # Z score for controls using leave-one-out
    Zscore[, idx[[control_idx]]] <-
      sapply(1:length(idx[[control_idx]]), function(x) {
        # calculate mean using leave-one out
        dataHealthSubset.mean <-
          t(apply(data[, idx[[control_idx]][-x]], 1, function(j) {
            GetMeanSD(as.numeric(j), measure)
          }))
        # calculate standard deviation using all of the control samples
        dataHealthSubset.sd <-
          t(apply(data[, idx[[control_idx]]], 1, function(j) {
            GetMeanSD(as.numeric(j), measure)
          }))
        
        dataHealthSubset <-
          cbind(dataHealthSubset.mean[, 1], dataHealthSubset.sd[, 2])
    ...
    Zscore.nonabsolute <-
      data.frame("Symbol" = rownames(Zscore), Zscore)
    Zscore <- abs(Zscore)
    ...
    if (save_tables == T) {
      write.table(
        x = Zscore.nonabsolute,
        file = file.path(path, "Zscore.tsv"),
        row.names = F
      )
    }
    ...
    Zgroups.list <-
      lapply(idx, function(x)
        rowSums(Zscore[, x, drop = FALSE]) / length(x)) # calculates gMDP for each gene in each class
    ...
    Zgroups.annotated <-
      data.frame("Symbol" = Zscore.annotated[, "Symbol"], Zgroups)
    ...
    Zgroups.np.list <-
      lapply(idx, function(x)
        rowSums((Zscore[, x, drop = FALSE] > 0)) / length(x)) # calculates gMDP for each gene in each class
    ...
    for (i in 1:dim(gMDP.score)[2]) {
      gene.df <-
        rbind(
          gene.df,
          data.frame(
            "Gene" = rownames(gMDP.score),
            "gMDP" = gMDP.score[, i],
            "Freq" = np.score[, i],
            "Disease" = gsub("gMDP_", "", colnames(gMDP.score)[i])
          )
        )
    }
    ...
    Zgroups.annotated <-
      Zgroups.annotated[, -grep(paste("score_", control_lab, sep = ""),
                                names(Zgroups.annotated))] # remove control score
    Zgroups.annotated <- Zgroups.annotated[order(-rowSums(score)), ]
    Zgroups.ranked <-
      cbind("order" = seq(1:nrow(Zgroups.annotated)), Zgroups.annotated)
    
   
    # find the top perturbed genes out of ALL genes
    genes <- Zgroups.ranked$Symbol
    percUsed <- round(perc * length(genes))
    genes <- genes[1:percUsed]
    ...
    for (s in 1:length(genesets)) {
      ...
      Zsamples <-
        colSums(Zscore.annotated.sub[, -c(1)], na.rm = T) / nrow(Zscore.annotated.sub)
      
      Zsamples.df <-
        data.frame(
          "Sample" = names(Zsamples),
          "sMDP" = Zsamples,
          "Class" = pdata[names(Zsamples), "Class"],
      ...
      Zsamples.df$Thresh <-
        rep(healthy.mean[1] + healthy.mean[[2]], length(Zsamples))
      
      # leave one out for healthy
      leave.out.mean <-
        sapply(1:length(idx[[control_idx]]), function(x, y)
          (GetMeanSD(y[idx[[control_idx]][-x], "sMDP"], measure)), y = Zsamples.df)
      Zsamples.df[idx[[control_idx]], "Thresh"] <-
        leave.out.mean[1, ] + healthy.mean[2]
      
      # mark perturbed samples
      Zsamples.df[Zsamples.df$sMDP >= Zsamples.df$Thresh, "Perturbed"] <-
        "Perturbed"
    ...
    if (save_tables == T) {
      write.table(
        x = Zgroups.annotated,
        file = file.path(path, "gMDPscore.tsv"),
        row.names = F
      )
    }
    if (print == TRUE) {
      ...
      Zgroups.plot <-
        data.frame(matrix(nrow = nGroups * ngenes, ncol = 4)) # construct a matrix of dim (ngenes*classes)*3, cols show gene Order, gMDP, Class,  frac perturbed
      
      for (i in 1:nGroups) {
        Zgroups.plot[(ngenes * (i - 1) + 1):(ngenes * i), 1:3] <-
          Zgroups.ranked[, c(1, i + 2, i + 2 + nGroups)]
        Zgroups.plot[(ngenes * (i - 1) + 1):(ngenes * i), 4] <-
          rep(colnames(Zgroups.ranked)[i + 2], ngenes)
      }
      ...
      test <-
        ...
        ) +  ggplot2::geom_jitter(alpha = 0.3, size = 3) +
        ...
        ggplot2::labs(title = "gMDP vs. fraction of samples gene perturbed in", x =
                        "Fraction of samples in which gene is perturbed", y = "gMDP score") + ggplot2::guides(alpha = FALSE)
      plot(test)
      dev.off()
    ...
    geneNumber <-
      lapply(1:length(genesets), function(x)
        length(genesets[[x]]))
    geneNumber <- unlist(geneNumber)
    
    overlapNumber <-
      lapply(1:length(genesets), function(x, y)
        sum(y %in% genesets[[x]]), y = Zscore.annotated$Symbol)
    ...
    pathwayMDP.df <-
      data.frame(
        ...
        "Rank" = rep(NA, length(genesets)),
        "Test.value" = rep(NA, length(genesets)),
        "Pathway.size" = geneNumber,
        "Genes.in.data" = overlapNumber,
        Zsamples.list
      )
    
    smdp.h <-
      as.matrix(Zsamples.list[, pdata$Sample[pdata$Class == control_lab]])
    smdp.p <-
      as.matrix(Zsamples.list[, pdata$Sample[pdata$Class != control_lab]])
    
    
    
    pathwayMDP.df$Test.value <-
      sapply(1:(dim(smdp.h)[1]), function(x)
        (mean(smdp.p[x, ]) - mean(smdp.h[x, ])) / (sd(smdp.h[x, ], na.rm = T) + sd(smdp.p[x, ], na.rm = T)))
    
    pathwayMDP.df$Rank <-
      rank(pathwayMDP.df$Test.value,
           ties.method = "min",
           na.last = "keep")
    ...
    if (save_tables == T) {
      write.table(
        x = pathwayMDP.df,
        file = file.path(path, "sMDPscores.tsv"),
        row.names = F
      )
    }
    ...
    if (!missing(pathways)) {
      if (print == TRUE) {
        pathwayMDP.df$Pathway <-
          factor(pathwayMDP.df$Pathway, levels = pathwayMDP.df$Pathway[order(pathwayMDP.df$Test.value)])
        
        
        pdf(file.path(path, "geneset_summary.pdf"))
        test <-
          ggplot2::ggplot(pathwayMDP.df, ggplot2::aes(x = Pathway, y = Test.value)) +  ggplot2::geom_bar(stat = "identity") +
    ...
    if (print == TRUE) {
      ...
      if (!missing(pathways)) {
        top.pathway <-
          pathwayMDP.df$Pathway[-match(c("allgenes", "perturbedgenes"), pathwayMDP.df$Pathway)][1]
        pathway.names <-
          c("allgenes",
            "perturbedgenes",
            as.character(top.pathway))
      } else {
      ...
      for (j in 1:length(pathway.names)) {
        smdp.plot(
          sMDP.data = sMDP.list[[pathway.names.idx[j]]],
          filename = pathway.names[j],
          directory = path,
          title.graph = pathway.names[j]
        )
      }
    ...
    # ---------------- OUTPUT ---------------- ####
    output <-
      list(Zscore.annotated,
           Zgroups.annotated,
           sMDP.list,
           pathwayMDP.df,
           genes)
    names(output) <-
      c("Zscore", "gMDP", "sMDP", "pathways", "perturbedgenes")
...
smdp.plot <-
  function(sMDP.data,
    ...
    Zsamples.plot$Sample <-
      factor(Zsamples.plot$Sample, levels = Zsamples.plot$Sample[order(Zsamples.plot$sMDP)])
    
    #Plot sMDP as bar graphs
    bp1 <-
      ggplot2::ggplot(data = Zsamples.plot, ggplot2::aes(y = sMDP, x = Sample, fill = Class)) +
    ...
    bp2 <-
      ggplot2::ggplot(data = Zsamples.plot, ggplot2::aes(y = sMDP, x = Class, fill = Class)) +
      ggplot2::geom_boxplot(outlier.shape = NA) + ggplot2::stat_summary(
        fun.y = mean,
        geom = "point",
        shape = 23,
        size = 6
